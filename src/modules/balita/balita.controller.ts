...
export const getAllBalitas = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await balitaService.findAll(page, limit, { villageId, poskoId });
  sendSuccess(res, 'Balitas retrieved successfully', result.balitas, result.meta);
});
...
export const createBalita = asyncHandler(async (req: Request, res: Response) => {
  const { namaAnak, namaOrtu, tanggalLahir, jenisKelamin, villageId, poskoId } = req.body;

  if (!namaAnak || !namaOrtu || !tanggalLahir || !jenisKelamin || !villageId) {
    throw new BadRequestError('namaAnak, namaOrtu, tanggalLahir, jenisKelamin, and villageId are required');
  }
