...
Bagi Anda yang ingin menjalankan aplikasi di lingkungan production menggunakan Docker. Sangat tidak disarankan untuk me-load _secrets/env_ secara langsung ke dalam Dockerfile maupun di-build ke image itu sendiri karena ini akan memicu resiko keamanan serius (Siapapun yang punya image Anda bisa melihat _secret variables_ seperti _database credentials_ atau _JWT key_).

Pendekatan _best practice_ adalah dengan mengirim env variables pada saat container di jalankan (lewat flag `--env-file`).

1. Buat file `.env.production` dari template yang disediakan:
   ```bash
   cp .env.production.example .env.production
   ```
2. Sesuaikan nilai di `.env.production` dengan environment database production Anda.
   > ⚠️ **PENTING:** Docker `--env-file` membaca nilai secara harfiah (literal). **JANGAN** menggunakan tanda kutip ganda (`"`) untuk mengapit _value_ string di dalam file ini seperti yang biasa dilakukan di file `.env` Node.js lokal Anda (Gunakan `DATABASE_URL=postgres://...` bukan `DATABASE_URL="postgres://..."`). Jika menggunakan tanda kutip, aplikasi akan sering kali mengalami error Host Not Found / DNS Exception saat dijalankan.
3. Build image Docker:
   ```bash
   docker build -t sigana-be .
   ```
4. Jalankan container dengan menyertakan file `.env.production` tersebut:
   ```bash
   docker run -d -p 3000:3000 --env-file .env.production --name sigana-api sigana-be
   ```

_Catatan: Pastikan Anda sudah menjalankan migrasi database (`bunx prisma migrate deploy` / `bunx prisma db push`) pada database production sebelum menjalankan container, agar aplikasi start dengan lancar._
